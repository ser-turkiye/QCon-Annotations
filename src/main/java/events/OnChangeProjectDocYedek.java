...
public class OnChangeProjectDocYedek extends UnifiedAgent {
    ...
    protected Object execute() {
        ...
        try {
          ...
          /*  if(getEventName().equals("createDocument")) {
            ...
            this.log.info("----OnChangeProjectDoc Agent Started ---for IDocument ID:--" + mainDocument.getID());
            IFolder prjFolder = this.getProjectFolder();
            if (prjFolder == null) {
                throw new Exception("Project folder not found.");
            }

            this.addToRootNode(prjFolder, "Project Documents", "Engineering Doc", mainDocument);

            List<String> nodeNames = new ArrayList<>();
            nodeNames.add(mainDocument.getDescriptorValue("ccmPrjDocOiginator"));
            nodeNames.add(mainDocument.getDescriptorValue("ccmPrjDocCategory"));
            if(mainDocument.getDescriptorValue("ccmPrjDocDiscipline") != null){nodeNames.add(mainDocument.getDescriptorValue("ccmPrjDocDiscipline"));}

            INode nNode = getNodesByList(nodeNames);
            if(nNode != null){
                this.addToNode(nNode, mainDocument);
            }
    ...
    public IDocument getEngDocument(ISession session, String prjCode, String docKey) throws IOException {
        ...
        IDescriptor descriptor2 = documentServer.getDescriptorForName(session, "ccmPrjDocNumber");
        ...
        searchDescriptors.put(descriptor1.getId(), prjCode);
