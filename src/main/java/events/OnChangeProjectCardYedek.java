...
public class OnChangeProjectCardYedek extends UnifiedAgent {
    ...
    protected Object execute() {
        ...
        try {
            ...
            updateRolesByPrjCard(mainDocument);
    ...
    public boolean existPRJGVList(String paramName, String key1) throws Exception {
        boolean rtrn = false;
        IStringMatrix settingsMatrix = getDocumentServer().getStringMatrix(paramName, getSes());
    ...
    public boolean existDCCGVList(String paramName, String key1, String key2) throws Exception {
        boolean rtrn = false;
        IStringMatrix settingsMatrix = getDocumentServer().getStringMatrix(paramName, getSes());
        String rowValuePrjCode = "";
    ...
    public void updateRolesByPrjCard(IDocument doc) throws Exception {
        try {
            ...
            if(emanagerUser != null){
                ...
                if(emanagerUser.getLicenseType() != LicenseType.NORMAL_USER) {
                    IUser cuser = emanagerUser.getModifiableCopy(getSes());
            ...
            if(pmanagerUser != null){
                ...
                if(pmanagerUser.getLicenseType() != LicenseType.NORMAL_USER) {
                    IUser cuser = pmanagerUser.getModifiableCopy(getSes());
                    cuser.setLicenseType(LicenseType.NORMAL_USER);
                    cuser.commit();
                }
    ...
    public void updateUnitsByPrjCard(String unitName, List<String> members){
        ...
        if(punit != null){
            ...
            if(unit != null){
                ...
                String[] unitIDs = new String[]{unit.getID()};
                for (String memberID : members) {
                    IUser memberUser = getDocumentServer().getUser(getSes(), memberID);
                    if (memberUser != null) {
                        if (!String.join(",", memberUser.getUnitIDs()).contains(unitIDs[0])) {
                            IUser cuser = memberUser.getModifiableCopy(getSes());
                            cuser.setUnitIDs(unitIDs);
                            cuser.commit();
                        }
                    }
                }
                IUser[] prjUnitMembers = (unit != null ? unit.getUserMembers() : null);
                if (prjUnitMembers != null) {
                    for (IUser pMember : prjUnitMembers) {
                        prjUnitUserIDs.add(pMember.getID());
                    }
                }
                for (int i = 0; i < prjUnitUserIDs.size(); i++) {
                    String prjUserID = prjUnitUserIDs.get(i);
                    IUser prjUnitUser = getDocumentServer().getUser(getSes(), prjUserID);
                    if (!members.contains(prjUserID)) {
                        String[] removedUserUnits = removeFromUnit(prjUnitUser, unitIDs[0]);
                        IUser cuser = prjUnitUser.getModifiableCopy(getSes());
                        cuser.setUnitIDs(removedUserUnits);
                        cuser.commit();
                        log.info("removed unit [" + removedUserUnits.toString() + "] from user:" + prjUnitUser.getFullName());
                    }
                }
    ...
    public String[] removeFromUnit(IUser user, String unitID){
